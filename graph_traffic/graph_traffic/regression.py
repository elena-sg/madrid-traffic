...
def train_with_args(data_dict, meteo_dict, temporal_dict, pipeline_class):
    mmagns = []
    if meteo_dict["rain"] != "drop":
        mmagns.append("precipitacion")
    if meteo_dict["wind"] != "drop":
    ...
    estimators = {}
    for sensor_id in ids_used:
        print(sensor_id)
        train_x = dfs_dict[sensor_id][:int(0.8 * data_size), 1:]
        train_y = dfs_dict[sensor_id][:int(0.8 * data_size), 0].ravel()

        test_x = dfs_dict[sensor_id][int(0.8 * data_size):, 1:]
        test_y = dfs_dict[sensor_id][int(0.8 * data_size):, 0].ravel()
    ...
    return ids_used, estimators, dfs_dict, test_dates
